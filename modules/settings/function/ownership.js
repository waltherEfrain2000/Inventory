...
$(function () {
    ...
    $("#navPertenencia").prop("style", "background-color: rgb(225 224 223); border-radius: 8px;");

    obtenerPertenencias();

    function listarPertenencias() {
        ...
        cargarTabla("#tabla", pertenencias, columns);
    }
    ...
    $("#guardar").on("click", function (e) {
        ...
        guardarPertenencia(data);
    });

    async function obtenerPertenencias() {
        try {
            const response = await fetch('./modules/settings/controller/ownershipController.php', {
                method: 'GET',
                headers: { 'Content-Type': 'application/json' },
            });
            if (response.ok) {
                pertenencias = await response.json(); 
                listarPertenencias();
            } else {
            ...
            Swal.fire({
                icon: "error",
                title: "Error",
                text: error || 'Ha ocurrido un error al obtener las pertenencias',
            });
        }
    }

    const guardarPertenencia = async (data) => {
        try {
            ...
            if (response.ok) {
                ...
                console.error('Error:', errorData.message || 'Error al guardar la pertenencia');
                Swal.fire({
                    icon: "error",
                    title: "Error",
                    text: errorData.message || 'Ha ocurrido un error al guardar la pertenencia',
                });
            }
        } catch (error) {
            console.log(error);
            Swal.fire({
                icon: "error",
                title: "Error",
                text: 'Ha ocurrido un error al guardar la pertenencia',
            });
        }
    }

    async function editarPertenencia(data) {
        try {
            const response = await fetch('./modules/settings/controller/ownershipController.php', {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (response.ok) {
                ...
                const errorData = await response.json();
                console.error('Error:', errorData.message || 'Error al actualizar la pertenencia');
                Swal.fire({
                    icon: "error",
                    title: "Error",
                    text: errorData.message || 'Ha ocurrido un error al actualizar la pertenencia',
                    target: "#editarModal"
                });
            }
        } catch (error) {
            ...
            Swal.fire({
                icon: "error",
                title: "Error",
                text: 'Ha ocurrido un error al actualizar la pertenencia',
                target: "#editarModal"
            });
        }
    }

    async function eliminarPertenencia(data) {
        try {
            const response = await fetch('./modules/settings/controller/ownershipController.php', {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
            ...
            if (response.ok) {
                ...
                const errorData = await response.json();
                console.error('Error:', errorData.message || 'Error al eliminar la pertenencia');
                Swal.fire({
                    icon: "error",
                    title: "Error",
                    text: errorData.message || 'Ha ocurrido un error al eliminar la pertenencia',
                });
            }
        } catch (error) {
            console.log(error);
            Swal.fire({
                icon: "error",
                title: "Error",
                text: 'Ha ocurrido un error al eliminar la pertenencia',
            });
        }
    }
});