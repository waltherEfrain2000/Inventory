import { apiFetch, cargarTabla, getParamFromUrl, generarSelect } from "./helpers.js";
...
$(function () {
    ...
    inicializarPantalla(idMantenimiento);
        ...
        if($("#catalogo").val() != ""){
            if(existeRegistro($("#catalogo").val(), trabajosARealizar)){
                ...
                trabajosARealizar.push({
                    id_catalogo: Number($("#catalogo").val()),
                    nombre_catalogo: $("#catalogo option:checked").text().trim(),
    ...
    $("#btnAgregarInsumo").on("click", function () {
        ...
        insumos.push({
            id_articulo: Number($("#insumos").val()),
            articulo: $("#insumos option:checked").text().trim(),
            unidad: $("#unidad").val(),
    ...
    $("#btnGuardar").on("click", function () {
        ...
        // Detectar trabajos nuevos (no estaban en el original)
        const trabajosNuevos = trabajosARealizar.filter(nuevo =>
            !trabajosOriginales.some(orig => orig.id === nuevo.id)
        );

        // Detectar trabajos eliminados (ya no están en el nuevo array)
        const trabajosEliminados = trabajosOriginales.filter(orig =>
            !trabajosARealizar.some(nuevo => nuevo.id === orig.id)
        );

        // Igual para insumos
        const insumosNuevos = insumos.filter(nuevo =>
            !insumosOriginales.some(orig => orig.id === nuevo.id)
        );

        const insumosEliminados = insumosOriginales.filter(orig =>
            !insumos.some(nuevo => nuevo.id === orig.id)
        );
        

        let data = {
            id_mantenimiento: idMantenimiento || null,
    ...
    async function inicializarPantalla(idMantenimiento) {
        try {
            await Promise.all([
                obtenerVehiculos(),
            ...
            if (idMantenimiento) {
                await obtenerMantenimiento(idMantenimiento);
            }
        } catch (error) {
    ...
    function cargarTrabajos() {
        ...
        const total = trabajosARealizar.reduce(
            (accumulator, currentValue) => accumulator + Number(currentValue.costo),
            0
        );
    ...
    function existeRegistro(id, objeto) {
        return objeto.some(function(item) {
            return item.id === id || item.id_catalogo === Number(id) || item.id_articulo === Number(id);
        });
    }
    ...
    const guardarMantenimiento = async (data) => {
        ...
        try {
            const result = await apiFetch('./modules/transport/controller/mantenimientoController.php', 'PUT', data);
            Swal.close();
    ...
    async function obtenerMantenimiento(idMantenimiento){
        try {
            ...
            if (result.success && !result.data.error) {
                ...
                if(mantenimiento.encabezado.estado != 1 && mantenimiento.encabezado.estado != 5) {
                    $("#btnGuardar").prop({'disabled': true, 'hidden': true}); 
                    return;
                }
    ...
    function renderizarMantenimiento(data) {
        isUserEvent = false;
        let encabezado = data.encabezado;
        trabajosARealizar = data.detalle;
        trabajosOriginales = [...data.detalle];
        insumos = data.insumos;
        insumosOriginales = [...data.insumos];

        $("#vehiculo").val(encabezado.id_vehiculo).trigger("change");
        $("#tipoMantenimiento").val(encabezado.id_tipo_mantenimiento).trigger("change");
        $("#tipoServicio").val(encabezado.id_tipo_servicio).trigger("change");
        ...
        if(encabezado.id_tipo_servicio == 2){
            $(".taller").prop("hidden", false);
            $("#taller").val(encabezado.id_taller).trigger("change");
        } else {
            $("#bodega").val(encabezado.id_bodega).trigger("change");
